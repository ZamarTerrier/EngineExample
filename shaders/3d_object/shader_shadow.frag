...
layout(binding = 1) uniform DirLightBuffer{
...
layout(binding = 3) uniform SpotLightBuffer{
    SpotLight spots[MAX_LIGHTS];
    LightMatrix mats[MAX_LIGHTS];
} slb;

layout(binding = 4) uniform LightStatusBuffer{
    int num_dirs;
    int num_points;
    int num_spots;
    int isEnable;
...
layout(binding = 5) uniform sampler2D shadowMap[MAX_SHADOWS];
layout(binding = 6) uniform samplerCube shadowMap2[2];
layout(binding = 7) uniform sampler2D shadowMap3[2];
layout(binding = 8) uniform sampler2D diffuse;
...
layout(location = 4) in vec4 inModel;
...
vec3 CalcSpotLightBase(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir, float shadow)
{
    vec3 lightDir = normalize(light.position - fragPos);
    // diffuse shading
    float diff = max(dot(normal, lightDir), 0.0);
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir, float shadow)
{
    ...
    if (SpotFactor > light.cutOff) {
        vec3 Color = CalcSpotLightBase(light, normal, fragPos, viewDir, shadow);
...
void main() {
    ...
    if(lsb.isEnable != 0)
    {
        ...
        for(int i =0;i < lsb.num_dirs;i++){
            ...
            fullColor += CalcDirLight(d_light, fragNormal, some_vec, color.rgb, shadow);
        ...
        for(int i =0;i < lsb.num_points;i++){
            ...
            fullColor += CalcPointLight(light, fragNormal, some_vec, L.xyz, shadow);
        }
        ...
        for(int i =0;i < lsb.num_spots;i++){
            ...
            fullColor += CalcSpotLight(light, fragNormal, some_vec, L.xyz, shadow);
        }

        resColor = vec4(fullColor, 1.0);
    }
    else
        resColor = color;

    resColor.rgb *= (shadow + 0.35);
